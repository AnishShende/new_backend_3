...
def analistAdd(request):
    if request.method == "POST":
        ...
        print(session)
        db.collection(u'backend_users').document(data['email']).set(data)
...
def claims(request):
    ...
    cases_data = []
    list_status = ['draft',  'Unprocessed', 'query', 'Approved', 'Reject',
                   'Discharge', 'Discharge Approved', 'Settled']
    values = {
        "draft": 0,
        "Unprocessed": 0,
        "query": 0,
...
def savestatus(request):
    ...
    return HttpResponse('redirect to emailer')
...
def bunny(request):
    ...
    if request.method == "POST":
        ...
        sendemail(sender, reciever, sub, sender_msg, Bcc, Cc,file)
